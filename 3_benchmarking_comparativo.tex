...
Il primo passo per raggiungere l'obiettivo di questo elaborato è stata la produzione di una tabella comparativa che raccogliesse i principali modelli di Action Anticipation proposti in letteratura tra il 2015 e il 2024, la quale ha richiesto un processo metodico di raccolta, normalizzazione e validazione dei dati. Le tabelle e i grafici riportati sono stati realizzati 
con le librerie \texttt{pandas} e \texttt{matplotlib} 
di Python.
...
Questo elaborato si propone, dunque, di aggiungere un maggior livello di analisi critica alla letteratura esistente. 
...
\begin{itemize}
    ...
    \item \textbf{Metrica}: metrica di valutazione adottata (Top-5 Accuracy, Mean Top-5 Recall, Mean-over-Classes (MoC), ecc.).
    \item \textbf{Risultati}: valori numerici riportati, con separazione tra Verb, Noun e Action ove disponibile.
    \item \textbf{Input}: modalità di input utilizzate (RGB, Optic Flow, feature pre-estratte, audio, ecc.).
...
\begin{itemize}
    ...
    \item $\tau = 1$ s
    \item \textbf{Metrica}: Top-5 Accuracy
\end{itemize}
...
\begin{figure}
    ...
    {\footnotesize \textit{Legenda input} (valida per tutte le tabelle presenti nel capitolo): 
    \textbf{RGB} = Segnael RGB video;
    \textbf{F} = Optical Flow; 
    \textbf{O} = Object Detections; 
    \textbf{PA} = Past Action Segments; 
    \textbf{HM} = Hand Masks; 
    \textbf{DP} = Detected People.}
\end{figure}
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.85\textwidth]{grafici/barchart_ek55_val.pdf}
    \caption{Confronto delle prestazioni tra modelli STA su EK-55 Val, $\tau = 1$ s, metrica Top-5 Accuracy.}
...
\begin{figure}
    ...
    \caption{Tabella dei modelli STA su EGTEA Gaze+ Test, $\tau = 0.5$ s, metrica Top-1 Accuracy.}
    \label{fig:tab_egtea_test}
\end{figure}
\begin{figure}[htbp]
    \centering
    \includegraphics[width=\textwidth]{grafici/barchart_egtea_test.pdf}
    \caption{Confronto delle prestazioni tra modelli su EGTEA Gaze+ Test, $\tau = 0.5$ s, metrica Top-1 Accuracy.}
...
\begin{itemize}
    \item \textbf{Task}: LTA
    \item \textbf{Dataset}: Breakfast Actions
    \item \textbf{Input}: I3D Features
    \item \textbf{Osservazione} = 20\%
    \item \textbf{Metrica}: Mean-over-Classes Accuracy (MoC)
\end{itemize}
...
I quattro gruppi misurano capacità diverse e non sono direttamente confrontabili, sebbene il loro confronto sia comunque informativo.
